...
const StyledCard = styled(Card)(({ theme }) => ({
  ...
  boxSizing: "border-box", // Ensure padding/borders are included
  display: "flex",
...
const StyledCardContent = styled(CardContent)({
  height: "120px", // Fixed height for username + description
  minHeight: "120px",
  maxHeight: "120px",
  padding: "12px",
...
export default function ResponsiveGrid({ userId, searchQuery = "" }) {
  ...
  React.useEffect(() => {
    console.log("Grid received searchQuery:", searchQuery);
    ...
    const fetchPosts = async () => {
      try {
        ...
        console.log("Posts fetched response:", response.data);
  ...
  const handleCreatePost = async () => {
    if (loading || !userName) {
      setError("User name not loaded. Please wait or try again.");
      return;
    }
    let imageUrl = null;
    if (imageFile) {
    ...
    <Box className="min-h-screen bg-gradient-to-b from-gray-100 to-gray-200 py-6 px-4 sm:px-6 lg:px-8">
      <Box className="max-w-3xl mx-auto mb-8 bg-white p-6 rounded-2xl shadow-lg">
        ...
        <StyledButton
          ...
          disabled={loading || (!userName && !imageFile)}
        >
          {loading ? <CircularProgress size={24} className="text-black" /> : "Create Post"}
        </StyledButton>
      </Box>
      {searchQuery && (
